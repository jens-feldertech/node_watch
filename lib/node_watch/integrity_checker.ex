defmodule NodeWatch.IntegrityChecker do
  ...
  def check_node_integrity(node, cached_result) do
    ...
    with {:ok, result} <- RPCClient.patch_post(node.chain, methods, node.url),
      true <- matching_result?(result, cached_result) do
  ...
  def get_initial_result(node) do
    methods =  @config[node.chain]
   ...
   case RPCClient.patch_post(node.chain, methods, node.url) do
      ...
      {:error, error} ->
        {:error, "Failed to get initial result for #{node.chain} node #{node.url}: #{inspect(error)}"}
   end
  end

  defp matching_result?(result, cached_result), do: result == cached_result
